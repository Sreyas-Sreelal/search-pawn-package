...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.languages.registerCompletionItemProvider({ language: 'json', pattern: '**/pawn.json' }, {
        async provideCompletionItems (document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
            const postline = position.line;
            const { text } = document.lineAt(postline);
            const currentLineReplaceRange = new vscode.Range(new vscode.Position(postline, 0), new vscode.Position(postline, text.length));
